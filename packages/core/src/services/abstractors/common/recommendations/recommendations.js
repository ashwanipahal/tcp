  ...
  getAppData: ({ pageType, categoryName, partNumber }) => {
    ...
    return executeExternalAPICall({
      ...
      body: {
        ...
        requestLocation,
        mboxParameters: {
          'entity.categoryId': categoryName || '', // Hardcoded value present since API not providing data as of now for homepage, would be dynamic for PDP and Checkout page
          'entity.id': partNumber ? `${partNumber}_${region.toUpperCase()}` : '', // Hardcoded value present since API not providing data as of now for homepage, would be dynamic for PDP and Checkout page
          pageType,
          region,
        },
      },
