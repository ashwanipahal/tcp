...
import { getModuleX, ModuleXCompositeDataAbstractor } from '../moduleXComposite';
...
it('ModuleX Abstractor | ModuleXCompositeDataAbstractor | getData', () => {
  ModuleXCompositeDataAbstractor.getData('4546bfa-522e-4b99-9976-1cdc5be8d418').then(data => {
...
it('ModuleX Abstractor | DataAbstractor | processData', () => {
  ...
  ModuleXCompositeDataAbstractor.processData(rawData).then(data => {
    expect(data).toMatchObject(processedData);
  });
});
